...
test('unit: TodoCreated inserts a new active row', async () => {
  ...
  await handleEvent(
    {
      ...
      payload: {
        ...
        description: 'Whole milk', dueDate: '2026-12-31',
        priority: 'High', createdAt: '2026-01-01T00:00:00.000Z',
      },
    },
    client,
  );
  ...
  assert.match(sql, /INSERT INTO todo_detail_projection/i);
...
test('unit: TodoCreated uses ON CONFLICT DO NOTHING (idempotent)', async () => {
  ...
  await handleEvent(
    {
      type: 'TodoCreated',
      payload: { todoId: 't1', listId: 'l1', title: 'Buy milk', description: '', dueDate: null, priority: 'Medium', createdAt: '2026-01-01T00:00:00.000Z' },
    },
    client,
  );
...
test('unit: TodoEdited updates title, description, and priority', async () => {
  const client = makeClient();
  await handleEvent(
    { type: 'TodoEdited', payload: { todoId: 't1', title: 'New title', description: 'New desc', priority: 'Low' } },
    client,
  );
  assert.equal(client.query.mock.calls.length, 1);
  const [sql, params] = client.query.mock.calls[0].arguments;
  assert.match(sql, /UPDATE todo_detail_projection/i);
  assert.match(sql, /title.*description.*priority/i);
  assert.deepEqual(params, ['t1', 'New title', 'New desc', 'Low']);
});

test('unit: TodoDueDateSet updates due_date', async () => {
  const client = makeClient();
  await handleEvent(
    { type: 'TodoDueDateSet', payload: { todoId: 't1', dueDate: '2026-06-15' } },
    client,
  );
  assert.equal(client.query.mock.calls.length, 1);
  const [sql, params] = client.query.mock.calls[0].arguments;
  assert.match(sql, /UPDATE todo_detail_projection SET due_date/i);
  assert.deepEqual(params, ['t1', '2026-06-15']);
});
...
test('unit: TodoCompleted sets status to completed', async () => {
  ...
  const [sql, params] = client.query.mock.calls[0].arguments;
  assert.match(sql, /UPDATE todo_detail_projection SET status/i);
  assert.match(sql, /completed/);
  assert.deepEqual(params, ['t1']);
});

test('unit: TodoReopened sets status to active', async () => {
  const client = makeClient();
  await handleEvent(
...
test('unit: TodoDeleted sets status to deleted (row kept for audit)', async () => {
  const client = makeClient();
  await handleEvent(
...
test('unit: TodoDeleted does NOT hard-delete the row', async () => {
  const client = makeClient();
  await handleEvent(
    { type: 'TodoDeleted', payload: { todoId: 't1' } },
    client,
...
test('unit: TodoListArchived removes all todos for that list', async () => {
  ...
  const [sql, params] = client.query.mock.calls[0].arguments;
  assert.match(sql, /DELETE FROM todo_detail_projection/i);
