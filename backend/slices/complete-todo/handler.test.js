...
test('unit: rejects when todo does not exist (no events)', async () => {
  ...
  await assert.rejects(
    () => handleCompleteTodo({ todoId: 'todo-1' }, { store, query: makeQuery() }),
    { name: 'ValidationError', message: 'Todo must be in Active or Overdue status' },
  );
...
test('unit: rejects when todo is already completed', async () => {
...
test('unit: completes an active todo', async () => {
  const store = makeStore({ events: [e('TodoCreated')], version: 1 });

  const result = await handleCompleteTodo({ todoId: 'todo-1' }, { store, query: makeQuery() });
  ...
  assert.equal(event.type, 'TodoCompleted');
  assert.equal(event.payload.todoId, 'todo-1');
  assert.ok(event.payload.completedAt, 'completedAt must be set');
  assert.equal(opts.expectedVersion, 1);
});

test('unit: completes an overdue todo', async () => {
...
test('unit: completes a reopened todo (active after reopen)', async () => {
  const store = makeStore({
    events: [e('TodoCreated'), e('TodoCompleted'), e('TodoReopened')],
    version: 3,
  });
...
test('unit: completedAt is a valid ISO 8601 datetime string', async () => {
  const store = makeStore({ events: [e('TodoCreated')], version: 1 });

  await handleCompleteTodo({ todoId: 'todo-1' }, { store, query: makeQuery() });

  const [event] = store.append.mock.calls[0].arguments;
  const parsed = new Date(event.payload.completedAt);
...
test('unit: passes optimistic concurrency version to store.append', async () => {
  const store = makeStore({ events: [e('TodoCreated')], version: 4 });

  await handleCompleteTodo({ todoId: 'todo-1' }, { store, query: makeQuery() });

  const [, opts] = store.append.mock.calls[0].arguments;
  assert.equal(opts.expectedVersion, 4);
});
